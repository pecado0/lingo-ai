...
# Замени 'main' на имя твоего файла, если оно отличается
...
def run_tests():
    ...
    # Получаем реальный user_id для тестов (чтобы БД не ругалась на внешний ключ)
    # Убедись, что тестовый юзер существует в БД
    test_user = check_user("test@example.com")
    if not test_user:
        print("❌ Тестовый пользователь не найден! Создай его сначала.")
        return
    user_id = test_user["id"]
    
    # ---------------------------------------------------------
    print("🟡 Тест 1: Запрос с ошибкой (отсутствует текст)")
    event_bad = {
        "httpMethod": "POST",
        "path": "/translate",
        "body": json.dumps({"user_id": user_id}) # Забыли передать text
    }
    response_bad = handler(event_bad, None)
    print(f"Ожидаем statusCode 400. Результат: {response_bad.get('statusCode')}")
    print(f"Ответ: {response_bad.get('body')}\n")

    # ---------------------------------------------------------
    print("🟡 Тест 2: Стандартный перевод (слово без контекста)")
    event_normal = {
        "httpMethod": "POST",
        "path": "/translate",
        "body": json.dumps({"user_id": user_id, "text": "smart"})
    }
    response_normal = handler(event_normal, None)
    print(f"Ожидаем statusCode 200. Результат: {response_normal.get('statusCode')}")
    # Выводим декодированный JSON для красоты
    if response_normal.get('statusCode') == 200:
        parsed_body = json.loads(response_normal['body'])
        print(f"Перевод: {parsed_body.get('translation')}\n")
    else:
        print(f"Ошибка: {response_normal.get('body')}\n")

    # ---------------------------------------------------------
    print("🟡 Тест 3: Перевод с контекстом")
    event_context = {
        "httpMethod": "POST",
        "path": "/translate",
        "body": json.dumps({
            "user_id": user_id, 
            "text": "bank", 
            "context": "I sat near the water"
        })
    }
    response_context = handler(event_context, None)
    print(f"Ожидаем statusCode 200. Результат: {response_context.get('statusCode')}")
    if response_context.get('statusCode') == 200:
        parsed_body = json.loads(response_context['body'])
        print(f"Перевод: {parsed_body.get('translation')}\n")
    else:
        print(f"Ошибка: {response_context.get('body')}\n")
