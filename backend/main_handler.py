...
def handle_translate(body: dict):
    ...
    if not context:
        print(f"[TIME] начало поиска в кэше слова {text}")
        t_cache = time.time()
        # Ищем слово в кэше
        cached_word = db.get_cached_word(word = text)
        print(f"[TIME] Поиск в кэше занял {time.time() - t_cache:.3f}")
        # Отработка, если слово найдено в кэше
        if cached_word:
            print(f"[TIME] Запрос на добавление слова {text} в словарь пользователя")
            t_uw = time.time()
            db.add_user_word(user_id = user_id, word = text)
            print(f"[TIME] Добавление слова {text} в словарь пользователя заняло {time.time() - t_uw:.3f}")
            return cached_word        
        # Если слово не найдено в кэше, обращаемся к LLM
        if cached_word == None:
            print(f"[TIME] Запрос на перевод слова {text} к LLM")
            t_llm = time.time()
            llm_response = ai.get_llm_response(text = text, context = context)
            print(f"[TIME] Запрос на перевод слова {text} к LLM занял {time.time() - t_llm:.3f}")
            if not llm_response:
                raise Exception("Ошибка при обращении к LLM.")

            # Добавляем слово в кэш
            print(f"[TIME] Запрос на добавление слова {text} в кэш")
            t_cache = time.time()
            db.add_cached_word(
                    word = text,
            ...
            print(f"[TIME] Запрос на добавление слова {text} в кэш занял {time.time() - t_cache:.3f}")
    ...
    if context:
        print(f"[TIME] Запрос на перевод слова {text} к LLM с конекстом {context}")
        t_llm = time.time()
        llm_response = ai.get_llm_response(text = text, context = context)
        print(f"[TIME] Запрос на перевод слова {text} к LLM с конекстом {context} занял {time.time() - t_llm:.3f}")
        if not llm_response:
            raise Exception("Ошибка при обращении к LLM.")
